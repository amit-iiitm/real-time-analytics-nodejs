...
io.on('connection', function(socket) {
  ...
  // a user has visited our page - add them to the visitorsData object
	socket.on('visitor-data',function(data){
		...
		//compute and send visitor data to dashboard when a new user visits our page
		io.emit('update-stats',computeStats());
});	
  socket.on('disconnect', function() {
    // a user has left our page - remove them from the visitorsData object
	delete visitorsData[socket.id];
	//compute and send the visitor data to the dashboard when a user leaves our page
	io.emit('update-stats',computeStats());
  });
});

//wrapper function to compute the statsand return a object with the updated status
